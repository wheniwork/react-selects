...
var React$1 = ( react$2 && react ) || react$2;
...
var AutosizeInput = React$1.createClass({
	displayName: 'AutosizeInput',

	propTypes: {
		className: React$1.PropTypes.string, // className for the outer element
		defaultValue: React$1.PropTypes.any, // default field value
		inputClassName: React$1.PropTypes.string, // className for the input element
		inputStyle: React$1.PropTypes.object, // css styles for the input element
		minWidth: React$1.PropTypes.oneOfType([// minimum width for input element
		React$1.PropTypes.number, React$1.PropTypes.string]),
		onChange: React$1.PropTypes.func, // onChange handler: function(newValue) {}
		placeholder: React$1.PropTypes.string, // placeholder text
		placeholderIsMinWidth: React$1.PropTypes.bool, // don't collapse size to less than the placeholder
		style: React$1.PropTypes.object, // css styles for the outer element
		value: React$1.PropTypes.any },
	// field value
	...
	render: function render() {
		...
		return React$1.createElement(
			...
			React$1.createElement('input', _extends({}, inputProps, { ref: 'input' })),
			React$1.createElement(
				'div',
				{ ref: 'sizer', style: sizerStyle },
				sizerValue
			),
			this.props.placeholder ? React$1.createElement(
				'div',
	...
	var renderOptions = function renderOptions(optionsSubset) {
		return optionsSubset.map(function (option, i) {
			if (isGroup$1(option)) {
				...
				var optionClass = index$2(optionClassName, {
					'Select-option': true,
		...
		value: function componentWillUpdate(nextProps, nextState) {
			...
			propertiesToSync.forEach(function (prop) {
				if (_this2.props[prop] !== nextProps[prop]) {
					_this2.setState(defineProperty({}, prop, nextProps[prop]));
				}
			});
		}
	}, {
...
var AsyncCreatable = function (_Component) {
	...
	function AsyncCreatable() {
		...
		classCallCheck(this, AsyncCreatable);

		for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
			args[_key] = arguments[_key];
		}

		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = AsyncCreatable.__proto__ || Object.getPrototypeOf(AsyncCreatable)).call.apply(_ref, [this].concat(args))), _this), _this.focus = function () {
			_this.select.focus();
		}, _temp), possibleConstructorReturn(_this, _ret);
	}

	createClass(AsyncCreatable, [{
		key: 'render',
		value: function render() {
			var _this2 = this;
				...
				function (asyncProps) {
					return React__default.createElement(
						...
						function (creatableProps) {
							return React__default.createElement(Select, _extends$1({}, reduce(asyncProps, reduce(creatableProps, {})), {
								...
								ref: function ref(_ref2) {
									_this2.select = _ref2;
									creatableProps.ref(_ref2);
									asyncProps.ref(_ref2);
								}
							}));
						}
					);
...
var Creatable = function (_Component) {
	...
	function Creatable() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Creatable.__proto__ || Object.getPrototypeOf(Creatable)).call.apply(_ref, [this].concat(args))), _this), _this.createNewOption = function () {
			...
			var _this$props2 = _this.props,
			    filterOptions$$1 = _this$props2.filterOptions,
			...
			if (isValidNewOption({ label: _this.inputValue })) {
				var _newOptionCreator = _this.props.newOptionCreator;


				var option = _newOptionCreator({
					label: _this.inputValue,
					labelKey: _this.labelKey,
					valueKey: _this.valueKey
				});

				// TRICKY Compare to all options (not just filtered options) in case option has already been selected).
				// For multi-selects, this would remove it from the filtered list.
				var _isOptionUnique2 = _this.isOptionUnique({
					option: option,
					options: excludeOptions.concat(filteredOptions)
				});

				if (_isOptionUnique2) {
					var prompt = promptTextCreator(_this.inputValue);

					_this._createPlaceholderOption = _newOptionCreator({
						label: prompt,
						labelKey: _this.labelKey,
						valueKey: _this.valueKey
					});

					filteredOptions.unshift(_this._createPlaceholderOption);
				}
			}

			return filteredOptions;
		}, _this.isOptionUnique = function (_ref2) {
			var option = _ref2.option,
			    options = _ref2.options;
			var isOptionUnique = _this.props.isOptionUnique;


			options = options || _this.select.filterFlatOptions();
		...
		}, _this.menuRenderer = function (params) {
			var menuRenderer$$1 = _this.props.menuRenderer;


			return menuRenderer$$1(_extends$1({}, params, {
				onSelect: _this.onOptionSelect,
				selectValue: _this.onOptionSelect
			}));
			...
			var focusedOption = _this.select.getFocusedOption();

			if (focusedOption && focusedOption === _this._createPlaceholderOption && shouldKeyDownEventCreateNewOption({ keyCode: event.keyCode })) {
				_this.createNewOption();
		...
		}, _this.onOptionSelect = function (option, event) {
			if (option === _this._createPlaceholderOption) {
				_this.createNewOption();
			} else {
				_this.select.selectValue(option);
			}
	...
	createClass(Creatable, [{
		...
		value: function render() {
			...
			var _props = this.props,
			    ...
			    restProps = objectWithoutProperties(_props, ['newOptionCreator', 'shouldKeyDownEventCreateNewOption']);
			...
			var props = _extends$1({}, restProps, {
				...
				ref: function ref(_ref3) {
					...
					if (_ref3) {
						_this2.labelKey = _ref3.props.labelKey;
						_this2.valueKey = _ref3.props.valueKey;
					}
				}
			});
...
function isValidNewOption(_ref5) {
	var label = _ref5.label;

	return !!label;
}

function newOptionCreator(_ref6) {
...
var Option = function (_Component) {
	...
	function Option() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Option.__proto__ || Object.getPrototypeOf(Option)).call.apply(_ref, [this].concat(args))), _this), _this.blockEvent = function (event) {
			event.preventDefault();
	...
	createClass(Option, [{
		...
		value: function render() {
			...
			var className = index$2(this.props.className, option.className);
...
var OptionGroup = function (_Component) {
	...
	function OptionGroup() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = OptionGroup.__proto__ || Object.getPrototypeOf(OptionGroup)).call.apply(_ref, [this].concat(args))), _this), _this.blockEvent = function (event) {
			event.preventDefault();
			event.stopPropagation();
			if (event.target.tagName !== 'A' || !('href' in event.target)) {
				return;
...
var Value = function (_Component) {
	...
	function Value() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Value.__proto__ || Object.getPrototypeOf(Value)).call.apply(_ref, [this].concat(args))), _this), _this.handleMouseDown = function (event) {
		...
		}, _this.handleTouchEndRemove = function (event) {
			// Check if the view is being dragged, In this case
			// we don't want to fire the click event (because the user only wants to scroll)
			if (_this.dragging) return;
			...
			if (_this.props.disabled || !_this.props.onRemove) return;
			return React__default.createElement(
				'span',
			...
			return _this.props.onClick || _this.props.value.href ? React__default.createElement(
				...
				'span',
				{ className: className, role: 'option', 'aria-selected': 'true', id: _this.props.id },
				_this.props.children
			);
		}, _temp), possibleConstructorReturn(_this, _ret);
	}
	...
	createClass(Value, [{
		...
		value: function render() {
			return React__default.createElement(
				'div',
				{ className: index$2('Select-value', this.props.value.className),
...
var Select = function (_Component) {
	...
	function Select() {
		...
		return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Select.__proto__ || Object.getPrototypeOf(Select)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
			...
			required: false
		}, _this.toggleTouchOutsideEvent = function (enabled) {
			if (enabled) {
			...
			if (_this.wrapper && !_this.wrapper.contains(event.target) && _this.menuContainer && !_this.menuContainer.contains(event.target)) {
				_this.closeMenu();
			}
		}, _this.focus = function () {
			...
			// Fire the mouse events
			_this.handleMouseDown(event);
		}, _this.handleTouchEndClearValue = function (event) {
			// Check if the view is being dragged, In this case
			// we don't want to fire the click event (because the user only wants to scroll)
			if (_this.dragging) return;
			...
			if (_this.props.disabled || event.type === 'mousedown' && event.button !== 0) {
				return;
			}
			...
			if (_this.state.isFocused) {
				...
				_this._openAfterFocus = _this.props.openOnFocus;
				_this.focus();
			}
		}, _this.handleMouseDownOnArrow = function (event) {
			// if the event was triggered by a mousedown and not the primary
			// button, or if the component is disabled, ignore it.
			if (_this.props.disabled || event.type === 'mousedown' && event.button !== 0) {
				return;
			...
			// close the menu
			_this.closeMenu();
		}, _this.handleMouseDownOnMenu = function (event) {
			// if the event was triggered by a mousedown and not the primary
			// button, or if the component is disabled, ignore it.
			if (_this.props.disabled || event.type === 'mousedown' && event.button !== 0) {
				return;
			...
			_this._openAfterFocus = true;
			_this.focus();
		}, _this.closeMenu = function () {
			if (_this.props.onCloseResetsInput) {
				_this.setState({
					isOpen: false,
					isPseudoFocused: _this.state.isFocused && !_this.props.multi,
					inputValue: ''
				});
			} else {
				_this.setState({
			...
			var isOpen = _this.state.isOpen || _this._openAfterFocus || _this.props.openOnFocus;
			...
			if (_this.state.inputValue !== event.target.value && _this.props.onInputChange) {
			...
			switch (event.keyCode) {
				...
				case 9:
					// tab
					if (event.shiftKey || !_this.state.isOpen || !_this.props.tabSelectsValue) {
						return;
					}
				...
				case 33:
					// page up
					_this.focusPageUpOption();
					break;
				case 34:
					// page down
					_this.focusPageDownOption();
					break;
				case 35:
					// end key
					if (event.shiftKey) {
			...
			event.preventDefault();
		}, _this.handleValueClick = function (option, event) {
			if (!_this.props.onValueClick) return;
			_this.props.onValueClick(option, event);
		}, _this.handleMenuScroll = function (event) {
			if (!_this.props.onMenuScrollToBottom) return;
			var target = event.target;

			if (target.scrollHeight > target.offsetHeight && !(target.scrollHeight - target.offsetHeight - target.scrollTop)) {
				_this.props.onMenuScrollToBottom();
			}
		}, _this.handleRequired = function (value, multi) {
			if (!value) return true;
			return multi ? value.length === 0 : Object.keys(value).length === 0;
		}, _this.getOptionLabel = function (op) {
			return op[_this.props.labelKey];
		}, _this.getValueArray = function (value, nextProps) {
			/** support optionally passing in the `nextProps` so `componentWillReceiveProps` updates will function as expected */
			var props = (typeof nextProps === 'undefined' ? 'undefined' : _typeof(nextProps)) === 'object' ? nextProps : _this.props;
			if (props.multi) {
			...
			var expandedValue = _this.expandValue(value, props);
			return expandedValue ? [expandedValue] : [];
		}, _this.expandValue = function (value, props) {
			var valueType = typeof value === 'undefined' ? 'undefined' : _typeof(value);
			if (valueType !== 'string' && valueType !== 'number' && valueType !== 'boolean') return value;
			var _this$props = _this.props,
			    labelKey = _this$props.labelKey,
			    valueKey = _this$props.valueKey,
			    renderInvalidValues = _this$props.renderInvalidValues;

			var options = _this._flatOptions;
			...
			if (renderInvalidValues) {
				...
				invalidOptions[value] = invalidOptions[value] || (_ref2 = {
					invalid: true
				}, defineProperty(_ref2, labelKey, value), defineProperty(_ref2, valueKey, value), _ref2);
			...
			if (_this.props.required) {
				var required = _this.handleRequired(value, _this.props.multi);
				_this.setState({ required: required });
			}
			if (_this.props.simpleValue && value) {
				value = _this.props.multi ? value.map(function (i) {
					return i[_this.props.valueKey];
				}).join(_this.props.delimiter) : value[_this.props.valueKey];
			}
			...
			if (_this.props.multi) {
				...
				_this.setState({
					isOpen: false,
					inputValue: '',
					isPseudoFocused: _this.state.isFocused
				}, function () {
					_this.setValue(value);
				});
			}
		}, _this.addValue = function (value) {
			var valueArray = _this.getValueArray(_this.props.value);
			var visibleOptions = _this._visibleOptions.filter(function (val) {
				return !val.disabled;
			});
			...
			if (visibleOptions.length - 1 === lastValueIndex) {
				...
				_this.focusOption(visibleOptions[lastValueIndex - 1]);
			...
			_this.setValue(valueArray.slice(0, valueArray.length - 1));
		...
		}, _this.getResetValue = function () {
			if (_this.props.resetValue !== undefined) {
				return _this.props.resetValue;
			} else if (_this.props.multi) {
				return [];
			} else {
				return null;
			}
		}, _this.focusOption = function (option) {
			_this.setState({
		...
		}, _this.focusPageUpOption = function () {
			_this.focusAdjacentOption('page_up');
		}, _this.focusPageDownOption = function () {
			_this.focusAdjacentOption('page_down');
		}, _this.focusStartOption = function () {
			_this.focusAdjacentOption('start');
		}, _this.focusEndOption = function () {
			_this.focusAdjacentOption('end');
		}, _this.focusAdjacentOption = function (dir) {
			var options = _this._visibleOptions.map(function (option, index) {
				return { option: option, index: index };
			}).filter(function (option) {
				return !option.option.disabled;
			});
			_this._scrollToFocusedOptionOnUpdate = true;
			if (!_this.state.isOpen) {
				_this.setState({
					isOpen: true,
					inputValue: '',
					focusedOption: _this._focusedOption || (options.length ? options[dir === 'next' ? 0 : options.length - 1].option : null)
				});
				return;
			}
			...
			var focusedIndex = -1;
			for (var i = 0; i < options.length; i++) {
				if (_this._focusedOption === options[i].option) {
					focusedIndex = i;
					break;
				}
			}
			if (dir === 'next' && focusedIndex !== -1) {
				...
				var potentialIndex = focusedIndex - _this.props.pageSize;
				if (potentialIndex < 0) {
					focusedIndex = 0;
				} else {
		...
		}, _this.getFocusedOption = function () {
			return _this._focusedOption;
		}, _this.getInputValue = function () {
			return _this.state.inputValue;
		}, _this.selectFocusedOption = function () {
			if (_this._focusedOption) {
				return _this.selectValue(_this._focusedOption);
			}
		}, _this.renderLoading = function () {
			if (!_this.props.isLoading) return;
			return React__default.createElement(
				'span',
				{ className: 'Select-loading-zone', 'aria-hidden': 'true' },
				React__default.createElement('span', { className: 'Select-loading' })
			);
		}, _this.renderValue = function (valueArray, isOpen) {
			var renderLabel = _this.props.valueRenderer || _this.getOptionLabel;
			var ValueComponent = _this.props.valueComponent;
			if (!valueArray.length) {
				return !_this.state.inputValue ? React__default.createElement(
					'div',
					{ className: 'Select-placeholder' },
					_this.props.placeholder
				) : null;
			}
			var onClick = _this.props.onValueClick ? _this.handleValueClick : null;
			if (_this.props.multi) {
				return valueArray.map(function (value, i) {
					return React__default.createElement(
						ValueComponent,
						{
							id: _this._instancePrefix + '-value-' + i,
							instancePrefix: _this._instancePrefix,
							disabled: _this.props.disabled || value.clearableValue === false,
							key: 'value-' + i + '-' + value[_this.props.valueKey],
							onClick: onClick,
				...
				return React__default.createElement(
					ValueComponent,
					{
						id: _this._instancePrefix + '-value-item',
		...
		}, _this.renderInput = function (valueArray, focusedOptionIndex) {
			var _classNames;

			var className = index$2('Select-input', _this.props.inputProps.className);
			var isOpen = !!_this.state.isOpen;

			var ariaOwns = index$2((_classNames = {}, defineProperty(_classNames, _this._instancePrefix + '-list', isOpen), defineProperty(_classNames, _this._instancePrefix + '-backspace-remove-message', _this.props.multi && !_this.props.disabled && _this.state.isFocused && !_this.state.inputValue), _classNames));

			// TODO: Check how this project includes Object.assign()
			var inputProps = Object.assign({}, _this.props.inputProps, {
				role: 'combobox',
				'aria-expanded': '' + isOpen,
				'aria-owns': ariaOwns,
				'aria-haspopup': '' + isOpen,
				'aria-activedescendant': isOpen ? _this._instancePrefix + '-option-' + focusedOptionIndex : _this._instancePrefix + '-value',
				...
				ref: function ref(_ref3) {
					return _this.input = _ref3;
				},
				required: _this.state.required,
				value: _this.state.inputValue
			});

			if (_this.props.inputRenderer) {
				return _this.props.inputRenderer(inputProps);
			}

			if (_this.props.disabled || !_this.props.searchable) {
				var _this$props$inputProp = _this.props.inputProps,
				    inputClassName = _this$props$inputProp.inputClassName,
				    divProps = objectWithoutProperties(_this$props$inputProp, ['inputClassName']);

				return React__default.createElement('div', _extends$1({}, divProps, {
					role: 'combobox',
					'aria-expanded': isOpen,
					'aria-owns': isOpen ? _this._instancePrefix + '-list' : _this._instancePrefix + '-value',
					'aria-activedescendant': isOpen ? _this._instancePrefix + '-option-' + focusedOptionIndex : _this._instancePrefix + '-value',
					className: className,
					tabIndex: _this.props.tabIndex || 0,
					onBlur: _this.handleInputBlur,
					onFocus: _this.handleInputFocus,
					ref: function ref(_ref4) {
						return _this.input = _ref4;
					},
					'aria-readonly': '' + !!_this.props.disabled,
					style: { border: 0, width: 1, display: 'inline-block' } }));
			}

			if (_this.props.autosize) {
				return React__default.createElement(AutosizeInput_1, _extends$1({}, inputProps, { minWidth: '5' }));
			}
			return React__default.createElement(
				'div',
				{ className: className },
			...
			if (!_this.props.clearable || !_this.props.value || _this.props.value === 0 || _this.props.multi && !_this.props.value.length || _this.props.disabled || _this.props.isLoading) return;
			var clear = _this.props.clearRenderer();

			return React__default.createElement(
				'span',
				{ className: 'Select-clear-zone',
					title: _this.props.multi ? _this.props.clearAllText : _this.props.clearValueText,
					'aria-label': _this.props.multi ? _this.props.clearAllText : _this.props.clearValueText,
			...
			var isOpen = _this.state.isOpen;
			var arrow = _this.props.arrowRenderer({ onMouseDown: onMouseDown, isOpen: isOpen });
		...
		}, _this.filterFlatOptions = function (excludeOptions) {
			var filterValue = _this.state.inputValue;
			var flatOptions = _this._flatOptions;
			if (_this.props.filterOptions) {
				// Maintain backwards compatibility with boolean attribute
				var filterOptions$$1 = typeof _this.props.filterOptions === 'function' ? _this.props.filterOptions : filterOptions;
				...
				return filterOptions$$1(flatOptions, filterValue, excludeOptions, {
					...
					labelKey: _this.props.labelKey,
					matchPos: _this.props.matchPos,
					matchProp: _this.props.matchProp,
					valueKey: _this.props.valueKey
				});
			} else {
				return flatOptions;
			}
		}, _this.flattenOptions = function (options, group) {
			if (!options) return [];
			var flatOptions = [];
			for (var i = 0; i < options.length; i++) {
				...
				if (isGroup(optionCopy)) {
					flatOptions = flatOptions.concat(_this.flattenOptions(optionCopy.options, optionCopy));
					optionCopy.options = [];
				} else {
			...
			return flatOptions;
		}, _this.unflattenOptions = function (flatOptions) {
			var groupedOptions = [];
			var parent = void 0,
			    child = void 0;
		...
		}, _this.onOptionRef = function (ref, isFocused) {
			if (isFocused) {
				_this.focused = ref;
			}
		}, _this.renderMenu = function (options, valueArray, focusedOption) {
			if (options && options.length) {
				return _this.props.menuRenderer({
					focusedOption: focusedOption,
					focusOption: _this.focusOption,
					instancePrefix: _this._instancePrefix,
					labelKey: _this.props.labelKey,
					onFocus: _this.focusOption,
					onOptionRef: _this.onOptionRef,
					onSelect: _this.selectValue,
					optionClassName: _this.props.optionClassName,
					optionComponent: _this.props.optionComponent,
					optionGroupComponent: _this.props.optionGroupComponent,
					optionRenderer: _this.props.optionRenderer || _this.getOptionLabel,
					options: options,
					selectValue: _this.selectValue,
					valueArray: valueArray,
					valueKey: _this.props.valueKey
				});
			} else if (_this.props.noResultsText) {
				return React__default.createElement(
					'div',
		...
		}, _this.renderHiddenField = function (valueArray) {
			if (!_this.props.name) return;
			if (_this.props.joinValues) {
				var value = valueArray.map(function (i) {
					return stringifyValue(i[_this.props.valueKey]);
				}).join(_this.props.delimiter);
				return React__default.createElement('input', {
					type: 'hidden',
					ref: function ref(_ref5) {
						return _this.value = _ref5;
					},
			...
			return valueArray.map(function (item, index) {
				return React__default.createElement('input', { key: 'hidden.' + index,
					...
					ref: 'value' + index,
					name: _this.props.name,
					value: stringifyValue(item[_this.props.valueKey]),
					disabled: _this.props.disabled });
			});
		}, _this.getFocusableOptionIndex = function (selectedOption) {
			var options = _this._visibleOptions;
			if (!options.length) return null;

			var focusedOption = _this.state.focusedOption || selectedOption;
			...
			var Dropdown$$1 = _this.props.dropdownComponent;
			...
			return React__default.createElement(
				...
				React__default.createElement(
					...
					{ ref: function ref(_ref7) {
							return _this.menuContainer = _ref7;
						}, className: 'Select-menu-outer',
						style: _this.props.menuContainerStyle },
					React__default.createElement(
						'div',
						{ ref: function ref(_ref6) {
								return _this.menu = _ref6;
							}, role: 'listbox', className: 'Select-menu', id: _this._instancePrefix + '-list',
   ...
   */
		...
		value: function render() {
			...
			var className = index$2('Select', this.props.className, {
